...
select distinct departamentos.nombre_depto from departamentos;
...
select nombre from empleados where id_depto = 3000;
...
select sal_emp,comision_emp,SUM(sal_emp+comision_emp),nombre from empleados where comision_emp > 1000 group by nombre order by nombre;

select sal_emp,comision_emp,SUM(sal_emp+comision_emp),nombre from empleados where comision_emp = 0 group by nombre order by nombre;

select nombre,sal_emp, comision_emp from empleados where sal_emp<comision_emp;

select nombre,sal_emp, comision_emp from empleados where (sal_emp*0.30)>=comision_emp;

select nombre from empleados where not nombre like '%MA%';

select nombre_depto from departamentos where nombre_depto = 'VENTAS' OR nombre_depto = 'INVESTIGACION'  OR nombre_depto = 'MANTENIMIENTO' group by nombre_depto;

select nombre_depto from departamentos where nombre_depto <> 'VENTAS' AND nombre_depto <> 'INVESTIGACION'  AND nombre_depto <> 'MANTENIMIENTO' group by nombre_depto;
...
SELECT nombre_depto, avg(sal_emp) FROM departamentos INNER JOIN empleados ON departamentos.id_depto = empleados.id_depto group by nombre_depto;

/*HAVING*/

select nombre_depto,count(id_emp) FROM departamentos INNER JOIN empleados ON departamentos.id_depto = empleados.id_depto group by nombre_depto having count(id_emp)>3;

select nombre FROM empleados INNER JOIN departamentos ON departamentos.id_depto = empleados.id_depto group by cod_jefe ;

SELECT nombre,nombre_depto FROM departamentos INNER JOIN empleados ON departamentos.id_depto = empleados.id_depto group by nombre_depto ,nombre;
